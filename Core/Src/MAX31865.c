...
#define BIT0(x)   ((x) & 0x01)       // Estrae il bit 0
#define BIT1(x)   (((x)>> 1) & 0x01) // Estrae il bit 1
...
void Max31865_readRegisterN(Max31865_t *max31865,uint8_t addr, uint8_t *buffer, uint8_t n)
{
	...
	HAL_GPIO_WritePin(DEMUX_A_GPIO_Port, DEMUX_A_Pin, BIT0(max31865->channel));
	HAL_GPIO_WritePin(DEMUX_B_GPIO_Port, DEMUX_B_Pin, BIT1(max31865->channel));
	HAL_GPIO_WritePin(max31865->cs_gpio, max31865->cs_pin, GPIO_PIN_RESET);
	max31865->spi->Instance->CR1 &= ~SPI_CR1_SPE;
	max31865->spi->Init.CLKPhase = SPI_PHASE_2EDGE;
    if (HAL_SPI_Init(max31865->spi) != HAL_OK)
    {
        /* Gestire l'errore, ad esempio con Error_Handler() */
        Error_Handler();
    }
	...
	while (n--)
	{
		HAL_SPI_TransmitReceive(max31865->spi, &tmp, buffer, 1, 100);//TR
		buffer++;
	}
	max31865->spi->Instance->CR1 &= ~SPI_CR1_SPE;
	max31865->spi->Init.CLKPhase = SPI_PHASE_1EDGE;
    if (HAL_SPI_Init(max31865->spi) != HAL_OK)
    {
        /* Gestire l'errore, ad esempio con Error_Handler() */
        Error_Handler();
...
uint16_t Max31865_readRTD (Max31865_t *max31865)
{
	...
	Max31865_writeRegister8(max31865, MAX31856_CONFIG_REG, t);
//	Max31865_delay(65);
	for(volatile uint32_t ll=0; ll<50000; ll++);
